...
def test_knight_creation(piece_color: PieceColor):
...
def test_knight_wrong_creation(piece_color):
...
@pytest.mark.parametrize("start, expected_end_position", [
    ...
    ((3, 7), [(4, 5), (5, 6), (2, 5), (1, 6)])])
def test_knight_get_moves(white_knight: Knight, start: tuple[int, int], expected_end_position: list[tuple[int, int]]):
